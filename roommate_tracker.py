...
if section == "➕ Log Expenses":
    ...
    for i in range(st.session_state.item_count):
        ...
        use_direct_cost = st.checkbox(f"Enter total cost directly for Item {i + 1}?", key=f"direct_{i}")

        if use_direct_cost:
            total_cost = st.number_input(f"Total Cost (SR) {i + 1}", min_value=0.0, format="%.2f", key=f"cost_direct_{i}")
            quantity = ""
            unit = ""
            rate = ""
        else:
            ...
            rate = st.number_input(f"Rate per {unit} (SR) {i + 1}", min_value=0.0, format="%.2f", key=f"rate_{i}")
